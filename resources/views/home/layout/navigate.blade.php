...
<div id="nav">
    <div id="nav_center">
        ...
        <a href="{{DOMAIN}}" class="{{$_SERVER['REQUEST_URI']=='/'?'curr':''}}">首 页</a>@foreach($navs as $nav)<a href="
            ...
            @endif
                " class="
            @if($nav['id']==1&&$_SERVER['REQUEST_URI']=='/s/free')curr
            @elseif($nav['id']==2&&$_SERVER['REQUEST_URI']=='/s/graph')curr
            @elseif($nav['id']==3&&$_SERVER['REQUEST_URI']=='/s/video')curr
            @elseif($nav['id']==4&&$_SERVER['REQUEST_URI']=='/s/design')curr
            @elseif($nav['id']==5&&$_SERVER['REQUEST_URI']=='/s/track')curr
            @endif
